	...
	<el-main class="bg edit_wrap comtable_e">
		...
		<el-row class="row_ce"> 
				...
				<el-form-item label="车主账号" prop="car_owner_account">
							...
							<!--v-if="user_group === '管理员' || (form['annual_inspection_appointment_id'] && $check_field('set','car_owner_account')) || (!form['annual_inspection_appointment_id'] && $check_field('add','car_owner_account'))" :disabled="disabledObj['car_owner_account_isDisabled']"></el-input>-->
							<!--<div v-else-if="$check_field('get','car_owner_account')">{{form['car_owner_account']}}</div>-->
							<el-select v-if="user_group === '管理员' || (form['annual_inspection_appointment_id'] && $check_field('set','car_owner_account')) || (!form['annual_inspection_appointment_id'] && $check_field('add','car_owner_account'))" id="car_owner_account" v-model="form['car_owner_account']" :disabled="disabledObj['car_owner_account_isDisabled']">
				...
				<el-form-item label="车主姓名" prop="owners_name">
							  ...
							  v-if="user_group === '管理员' || (form['annual_inspection_appointment_id'] && $check_field('set','owners_name')) || (!form['annual_inspection_appointment_id'] && $check_field('add','owners_name'))" :disabled="disabledObj['owners_name_isDisabled']"></el-input>
				...
				<el-form-item label="车主性别" prop="vehicle_owner_gender">
							  ...
							  v-if="user_group === '管理员' || (form['annual_inspection_appointment_id'] && $check_field('set','vehicle_owner_gender')) || (!form['annual_inspection_appointment_id'] && $check_field('add','vehicle_owner_gender'))" :disabled="disabledObj['vehicle_owner_gender_isDisabled']"></el-input>
				...
				<el-form-item label="车主电话" prop="car_owners_phone_number">
							  ...
							  v-if="user_group === '管理员' || (form['annual_inspection_appointment_id'] && $check_field('set','car_owners_phone_number')) || (!form['annual_inspection_appointment_id'] && $check_field('add','car_owners_phone_number'))" :disabled="disabledObj['car_owners_phone_number_isDisabled']"></el-input>
				...
				<el-form-item label="身份证号" prop="id_number">
							  ...
							  v-if="user_group === '管理员' || (form['annual_inspection_appointment_id'] && $check_field('set','id_number')) || (!form['annual_inspection_appointment_id'] && $check_field('add','id_number'))" :disabled="disabledObj['id_number_isDisabled']"></el-input>
				...
				<el-form-item label="车辆车型" prop="vehicle_model">
							  ...
							  v-if="user_group === '管理员' || (form['annual_inspection_appointment_id'] && $check_field('set','vehicle_model')) || (!form['annual_inspection_appointment_id'] && $check_field('add','vehicle_model'))" :disabled="disabledObj['vehicle_model_isDisabled']"></el-input>
				...
				<el-form-item label="车牌号码" prop="license_plate_number">
							  ...
							  v-if="user_group === '管理员' || (form['annual_inspection_appointment_id'] && $check_field('set','license_plate_number')) || (!form['annual_inspection_appointment_id'] && $check_field('add','license_plate_number'))" :disabled="disabledObj['license_plate_number_isDisabled']"></el-input>
				...
				<el-form-item label="店铺名称" prop="store_name">
							  ...
							  v-if="user_group === '管理员' || (form['annual_inspection_appointment_id'] && $check_field('set','store_name')) || (!form['annual_inspection_appointment_id'] && $check_field('add','store_name'))" :disabled="disabledObj['store_name_isDisabled']"></el-input>
				...
				<el-form-item label="检测地址" prop="detection_address">
							  ...
							  v-if="user_group === '管理员' || (form['annual_inspection_appointment_id'] && $check_field('set','detection_address')) || (!form['annual_inspection_appointment_id'] && $check_field('add','detection_address'))" :disabled="disabledObj['detection_address_isDisabled']"></el-input>
				...
				<el-form-item label="可约时间" prop="available_time_for_scheduling">
								<el-date-picker :disabled="disabledObj['available_time_for_scheduling_isDisabled']" v-if="user_group === '管理员' || (form['annual_inspection_appointment_id'] && $check_field('set','available_time_for_scheduling')) || (!form['annual_inspection_appointment_id'] && $check_field('add','available_time_for_scheduling'))" id="available_time_for_scheduling"
				...
				<el-form-item label="检测价格" prop="testing_prices">
						...
						v-if="user_group === '管理员' || (form['annual_inspection_appointment_id'] && $check_field('set','testing_prices')) || (!form['annual_inspection_appointment_id'] && $check_field('add','testing_prices'))" :disabled="disabledObj['testing_prices_isDisabled']"></el-input-number>
				...
				<el-form-item label="预约时间" prop="appointment_time">
								<el-date-picker :disabled="disabledObj['appointment_time_isDisabled']" v-if="user_group === '管理员' || (form['annual_inspection_appointment_id'] && $check_field('set','appointment_time')) || (!form['annual_inspection_appointment_id'] && $check_field('add','appointment_time'))" id="appointment_time"
				...
				<el-form-item label="检测内容" prop="detection_content">
							  ...
							  v-if="user_group === '管理员' || (form['annual_inspection_appointment_id'] && $check_field('set','detection_content')) || (!form['annual_inspection_appointment_id'] && $check_field('add','detection_content'))" :disabled="disabledObj['detection_content_isDisabled']"></el-input>
				...
				<el-form-item label="订单编号" prop="order_number">
							  ...
							  v-if="user_group === '管理员' || (form['annual_inspection_appointment_id'] && $check_field('set','order_number')) || (!form['annual_inspection_appointment_id'] && $check_field('add','order_number'))" :disabled="true"></el-input>
			...
			<el-col :xs="24" :sm="12" :lg="8" class="el_form_btn_warp">
				<el-form-item v-if="$check_action('/annual_inspection_appointment/view','set') || $check_action('/annual_inspection_appointment/view','add') || $check_option('/annual_inspection_appointment/table','examine')">
	...
	export default {
		...
		data() {
			return {
				...
				url_add: "~/api/annual_inspection_appointment/add?",
				url_set: "~/api/annual_inspection_appointment/set?",
				url_get_obj: "~/api/annual_inspection_appointment/get_obj?",
				url_upload: "~/api/annual_inspection_appointment/upload?",

				query: {
					"annual_inspection_appointment_id": 0,
				},
				...
				form: {
								"car_owner_account": 0, // 车主账号
									...
									"examine_state": "未审核",
							"examine_reply": "",
							"annual_inspection_appointment_id": 0, // ID
		...
		methods: {
			...
			is_view(){
				...
				if(!bl){
					bl = this.$check_action('/annual_inspection_appointment/table','add');
					console.log(bl ? "你有表格添加权限视作有添加权限" : "你没有表格添加权限");
				}
				if(!bl){
					bl = this.$check_action('/annual_inspection_appointment/table','set');
					console.log(bl ? "你有表格添加权限视作有修改权限" : "你没有表格修改权限");
				}
				if(!bl){
					bl = this.$check_action('/annual_inspection_appointment/view','add');
					console.log(bl ? "你有视图添加权限视作有添加权限" : "你没有视图添加权限");
				}
				if(!bl){
					bl = this.$check_action('/annual_inspection_appointment/view','set');
					console.log(bl ? "你有视图修改权限视作有修改权限" : "你没有视图修改权限");
				}
				if(!bl){
					bl = this.$check_action('/annual_inspection_appointment/view','get');
