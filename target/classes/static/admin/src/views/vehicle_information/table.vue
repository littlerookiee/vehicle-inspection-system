		...
		<el-table :data="list" @selection-change="selectionChange" @sort-change="$sortChange" style="width: 100%" id="dataTable">
			<el-table-column fixed type="selection" tooltip-effect="dark" width="55">
				...
				v-if="user_group == '管理员' || $check_field('get','frame_number')" min-width="200">
					...
					<el-table-column prop="driving_documents" @sort-change="$sortChange" label="驾驶证件"
				v-if="user_group == '管理员' || $check_field('get','driving_documents')" min-width="200">
						<template slot-scope="scope">
					<el-image style="width: 100px; height: 100px" :src="$fullUrl(scope.row['driving_documents'])"
						:preview-src-list="[$fullUrl(scope.row['driving_documents'])]">
					...
					<el-table-column prop="other_remarks" @sort-change="$sortChange" label="其他备注"
				v-if="user_group == '管理员' || $check_field('get','other_remarks')" min-width="200">
					</el-table-column>
			...
			<el-table-column fixed="right" label="操作" min-width="200" v-if="user_group == '管理员' || $check_action('/vehicle_information/table','set') || $check_action('/vehicle_information/view','set') || $check_action('/vehicle_information/view','get') || $check_action('/记录/table','add') || $check_action('/记录/view','add')" >
				...
				<template slot-scope="scope">
					...
					v-if="user_group == '管理员' || $check_action('/vehicle_information/table','set') || $check_action('/vehicle_information/view','set') || $check_action('/vehicle_information/view','get')"
						...
						<!--跨表按钮-->
							<el-button class="e-button el-button--small is-plain el-button--default" style="margin: 5px !important;" size="small" @click="to_table(scope.row,'/maintenance_records/view')" v-if="user_group == '管理员' || $check_action('/maintenance_records/table','add') || $check_action('/maintenance_records/view','add')">
						<span>记录</span>
					</el-button>
							</div>
				</template>
