		...
		<el-row class="row_ce"> 
			...
			<el-col :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				...
				<el-form-item label="用户姓名" prop="user_name">
							  ...
							  v-if="user_group === '管理员' || (form_sub['detecting_users_id'] && $check_field('set','user_name')) || (!form_sub['detecting_users_id'] && $check_field('add','user_name'))"></el-input>
				...
				<el-form-item label="用户性别" prop="user_gender">
						...
						v-if="user_group === '管理员' || (form_sub['detecting_users_id'] && $check_field('set','user_gender')) || (!form_sub['detecting_users_id'] && $check_field('add','user_gender'))">
				...
				<el-form-item label="用户电话" prop="user_phone_number">
						...
						v-if="user_group === '管理员' || (form_sub['detecting_users_id'] && $check_field('set','user_phone_number')) || (!form_sub['detecting_users_id'] && $check_field('add','user_phone_number'))">
					</el-input>
				...
				<el-form-item label="身份证号" prop="id_number">
							  ...
							  v-if="user_group === '管理员' || (form_sub['detecting_users_id'] && $check_field('set','id_number')) || (!form_sub['detecting_users_id'] && $check_field('add','id_number'))"></el-input>
			...
			<el-col :xs="24" :sm="12" :lg="8" class="el_form_btn_warp">
				<el-form-item v-if="$check_action('/detecting_users/view','set') || $check_action('/detecting_users/view','add') || $check_option('/detecting_users/table','examine')">
...
<script>
	...
	export default {
        ...
        form_sub = this.events("submit_before", Object.assign({}, form_sub)) || form_sub;
				if(form_sub["detecting_users_id"]){
					// 提交事件
					this.$post("~/api/" + this.group_table + "/set?detecting_users_id=" + form_sub["detecting_users_id"],form_sub,(res)=>{
			...
			is_view(){
				...
				if(!bl){
					bl = this.$check_action('/detecting_users/table','add');
					console.log(bl ? "你有表格添加权限视作有添加权限" : "你没有表格添加权限");
				}
				if(!bl){
					bl = this.$check_action('/detecting_users/table','set');
					console.log(bl ? "你有表格添加权限视作有修改权限" : "你没有表格修改权限");
				}
				if(!bl){
					bl = this.$check_action('/detecting_users/view','add');
					console.log(bl ? "你有视图添加权限视作有添加权限" : "你没有视图添加权限");
				}
				if(!bl){
					bl = this.$check_action('/detecting_users/view','set');
					console.log(bl ? "你有视图修改权限视作有修改权限" : "你没有视图修改权限");
				}
				if(!bl){
					bl = this.$check_action('/detecting_users/view','get');
