		...
		<el-row class="row_ce"> 
			...
			<el-col :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				...
				<el-form-item label="车主姓名" prop="owners_name">
												<el-input id="owners_name" v-model="form_sub['owners_name']" placeholder="请输入车主姓名"
							  v-if="user_group === '管理员' || (form_sub['car_owners_and_users_id'] && $check_field('set','owners_name')) || (!form_sub['car_owners_and_users_id'] && $check_field('add','owners_name'))"></el-input>
					<div v-else-if="$check_field('get','owners_name')">{{form_sub['owners_name']}}</div>
											</el-form-item>
			</el-col>
				...
				<el-form-item label="车主性别" prop="vehicle_owner_gender">
								<el-select id="vehicle_owner_gender" v-model="form_sub['vehicle_owner_gender']"
						v-if="user_group === '管理员' || (form_sub['car_owners_and_users_id'] && $check_field('set','vehicle_owner_gender')) || (!form_sub['car_owners_and_users_id'] && $check_field('add','vehicle_owner_gender'))">
						<el-option v-for="o in list_vehicle_owner_gender" :key="o" :label="o" :value="o">
						</el-option>
					</el-select>
					<div v-else-if="$check_field('get','vehicle_owner_gender')">{{form_sub['vehicle_owner_gender']}}</div>
							</el-form-item>
			</el-col>
				...
				<el-form-item label="车主电话" prop="car_owners_phone_number">
								<el-input id="car_owners_phone_number" v-model="form_sub['car_owners_phone_number']" placeholder="请输入车主电话" type="tel"
						v-if="user_group === '管理员' || (form_sub['car_owners_and_users_id'] && $check_field('set','car_owners_phone_number')) || (!form_sub['car_owners_and_users_id'] && $check_field('add','car_owners_phone_number'))">
					</el-input>
					<div v-else-if="$check_field('get','car_owners_phone_number')">{{form_sub['car_owners_phone_number']}}</div>
							</el-form-item>
			</el-col>
				...
				<el-form-item label="身份证号" prop="id_number">
							  ...
							  v-if="user_group === '管理员' || (form_sub['car_owners_and_users_id'] && $check_field('set','id_number')) || (!form_sub['car_owners_and_users_id'] && $check_field('add','id_number'))"></el-input>
			...
			<el-col :xs="24" :sm="12" :lg="8" class="el_form_btn_warp">
				<el-form-item v-if="$check_action('/car_owners_and_users/view','set') || $check_action('/car_owners_and_users/view','add') || $check_option('/car_owners_and_users/table','examine')">
	...
	export default {
		...
		data() {
			return {
				...
				group_table: "car_owners_and_users",
				is_password: true,
		...
		methods: {
			...
			async submit_check(param){
				...
				var form_sub = Object.assign({} ,this.form_sub);
																																								let car_owners_phone_number_phone_regular = /^(13[0-9]|14[01456879]|15[0-35-9]|16[2567]|17[0-8]|18[0-9]|19[0-35-9])\d{8}$/;
				if(form_sub.car_owners_phone_number && !car_owners_phone_number_phone_regular.test(form_sub.car_owners_phone_number)){
					return "手机号格式错误"
				}
																											let id_number_id_regular = /^[1-9]\d{5}(18|19|20)\d{2}((0[1-9])|(1[0-2]))(([0-2][1-9])|10|20|30|31)\d{3}[0-9Xx]$/;
				if(form_sub.id_number && !id_number_id_regular.test(form_sub.id_number)){
					return "身份证号格式错误"
				}
        ...
        form_sub = this.events("submit_before", Object.assign({}, form_sub)) || form_sub;
				if(form_sub["car_owners_and_users_id"]){
					// 提交事件
					this.$post("~/api/" + this.group_table + "/set?car_owners_and_users_id=" + form_sub["car_owners_and_users_id"],form_sub,(res)=>{
			...
			is_view(){
				...
				if(!bl){
					bl = this.$check_action('/car_owners_and_users/table','add');
					console.log(bl ? "你有表格添加权限视作有添加权限" : "你没有表格添加权限");
				}
				if(!bl){
					bl = this.$check_action('/car_owners_and_users/table','set');
					console.log(bl ? "你有表格添加权限视作有修改权限" : "你没有表格修改权限");
				}
				if(!bl){
					bl = this.$check_action('/car_owners_and_users/view','add');
					console.log(bl ? "你有视图添加权限视作有添加权限" : "你没有视图添加权限");
				}
				if(!bl){
					bl = this.$check_action('/car_owners_and_users/view','set');
					console.log(bl ? "你有视图修改权限视作有修改权限" : "你没有视图修改权限");
				}
				if(!bl){
					bl = this.$check_action('/car_owners_and_users/view','get');
